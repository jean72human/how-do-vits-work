...
from timm.models.sinkhorn import SinkhornDistance
...
class EigenLinear(nn.Module):
    def __init__(self, in_features, out_features, sign=-1, bias=False):
        super(EigenLinear, self).__init__()
    ...
    def forward(self, input):
        ...
        W = self.sign*self.weight@self.weight.transpose(-1,-2)

        # Perform Linear operation with sd matrix
...
class SymLinear(nn.Module):
    ...
    def forward(self, input):
        # Compute matrix product
        W = 0.5*(self.weight+self.weight.transpose(-1,-2))

        # Perform Linear operation with sd matrix
        return nn.functional.linear(input, W, self.bias)

class GraffLinear(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        super(GraffLinear, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.weight = nn.Parameter(torch.Tensor(out_features, in_features))
        if bias:
    ...
    def forward(self, input, othermatrix=None,q=-0.7,r=0.7):
        # Compute matrix product
        W = self.weight*(1 - torch.eye(self.out_features, device=self.weight.device))
        if othermatrix is not None:
            W_aux = (self.weight@othermatrix)*(1 - torch.eye(self.out_features, device=self.weight.device))
        else:
            W_aux = W
        W = W + torch.diag(q*torch.abs(W_aux).sum(-1)+r)
...
class AttentionMime(nn.Module):
    ...
    def __init__(
        ...
        a1,b1,a2,b2 = 0.7,0.7,0.4,0.4
        Z1,Z2 = torch.normal(torch.zeros((dim,dim)),(1/dim)*torch.eye(dim)),torch.normal(torch.zeros((dim,dim)),(1/dim)*torch.eye(dim))
        u1, s1, v1 = torch.svd(a1*Z1 + b1*torch.eye(dim))
        new_qkv_weight = torch.zeros_like(self.qkv.weight)
        new_qkv_weight[:dim,:] = u1@torch.sqrt(torch.diag(s1))
        new_qkv_weight[dim:2*dim,:] = v1@torch.sqrt(torch.diag(s1))
        u2, s2, v2 = torch.svd(a2*Z2 - b2*torch.eye(dim))
        new_qkv_weight[2*dim:,:] = u2@torch.sqrt(torch.diag(s2))
        self.qkv.weight.data = new_qkv_weight
        self.proj.weight.data = v2@torch.sqrt(torch.diag(s2))
...
class AttentionEigenNeg(nn.Module):
    fused_attn: Final[bool]
    ...
    def __init__(
        ...
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.Q = nn.Parameter(torch.Tensor(dim, dim))
        self.V = nn.Parameter(torch.Tensor(dim))
        self.proj_drop = nn.Dropout(proj_drop)

        nn.init.kaiming_uniform_(self.Q, a=math.sqrt(5))
        nn.init.uniform_(self.V, -1, 0)
        

    def forward(self, x):
        B, N, C = x.shape
        Q,_ = torch.linalg.qr(self.Q,mode="complete")
        V = -torch.diag(self.V)**2
        qk = self.qk(x).reshape(B, N, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k = qk.unbind(0)
        q, k = self.q_norm(q), self.k_norm(k)
        v = nn.functional.linear(x, Q).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        x = x.transpose(1, 2).reshape(B, N, C)
        x = nn.functional.linear(x, V@Q.T)
        x = self.proj_drop(x)
        return x
...
class AttentionEigenPos(nn.Module):
    fused_attn: Final[bool]
...
class Block(nn.Module):
    ...
    def forward_a(self,x):
        a = self.attn.forward_a(x)
        return a
    

class BlockMime(nn.Module):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            block_fn: Union[Callable, Tuple[Callable,Callable]] = Block,
    ...
    def forward_features(self, x):
        ...
        if not self.attach_first:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x, _ = checkpoint_seq(self.blocks, (x, x))
            else:
                x, _ = self.blocks((x, x))
        x = self.norm(x)
        return x
...
@register_model
def vit_tinyeigenpos_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('vit_tiny_patch16_224', pretrained=pretrained, block_fn=BlockEigenPos, **dict(model_args, **kwargs))
    return model

@register_model
def vit_tinyeigenneg_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('vit_tiny_patch16_224', pretrained=pretrained, block_fn=BlockEigenNeg, **dict(model_args, **kwargs))
    return model
...
@register_model
def vit_eigenpos_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=12)
    model = _create_vision_transformer('vit_base_patch32_224', pretrained=pretrained, block_fn=BlockEigenPos, weight_sharing=False, **dict(model_args, **kwargs))
    return model

@register_model
def vit_eigenneg_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=12)
    model = _create_vision_transformer('vit_base_patch32_224', pretrained=pretrained, block_fn=BlockEigenNeg, weight_sharing=False, **dict(model_args, **kwargs))
    return model

@register_model
def vit_mime_patch16_224(pretrained=False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=12)
    model = _create_vision_transformer('vit_base_patch32_224', pretrained=pretrained, block_fn=BlockMime, weight_sharing=False, **dict(model_args, **kwargs))
    return model
